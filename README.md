...
- Node.js 20 or newer (any version — see note below for why this used to matter and no longer does)
- npm

No external database service is needed — it uses a local SQLite file.

> **A crash we used to hit, now permanently fixed**: Node 24.19.0 introduced a core regression that crashed native addons built the old way (`Assertion failed: (env) != nullptr`) — this project's database engine, `better-sqlite3`, used to be affected. As of `better-sqlite3` v13, it was rewritten to use Node's N-API, which is structurally immune to that bug regardless of Node version. If you ever see that exact assertion error, check that `better-sqlite3` in `package.json` is on v13 or later — an older version reintroduces the old crash on affected Node releases (24.19.0 through at least 24.21.0, per Node's own tracking issue).
...
- **FCFA as the default currency**: prices display as whole numbers with a thousands separator ("25 000 FCFA") rather than decimals — the more natural convention for FCFA. Change `currency_symbol` in Admin → Contenu du site if you ever need a different currency (decimals return automatically for any symbol other than "FCFA").
- **Admin activity log** (Admin → Activité): a running history of who did what — logins, products created/edited/deleted, order status changes, content edits, and admin accounts added/removed. Each entry shows the actor, their role, and a plain-French description. Filterable by action type. The table auto-prunes to the most recent 2000 entries so it doesn't grow forever.
- **More admin customization**: a color picker to override the site's accent color (buttons, sale badges) site-wide without touching code, plus a "Confiance & livraison" section (delivery info, payment methods, return policy) that now feeds a real trust-signal section on the homepage — addressing the "add trust signals" suggestion from the security review.
- **Animations & micro-interactions**: buttons lift and give press feedback, product images zoom slightly on hover, the cart icon "bumps" when an item is added, the add-to-cart button flashes a success state, admin modals animate open/closed instead of snapping, and homepage sections fade in as you scroll to them. All respect `prefers-reduced-motion` for visitors who've asked their system to minimize animation.
- **WhatsApp redirect after checkout**: on the order confirmation page, if `contact_whatsapp` is set in Admin → Contenu du site, customers see their order reference for a few seconds, then get automatically redirected to a WhatsApp chat with your shop — with a message already filled in mentioning their order reference, so you instantly know which order they're asking about. A "Continuer maintenant" button lets them skip the wait. If `contact_whatsapp` is left empty, this is skipped entirely and the page behaves as before — nothing breaks either way.
