...
router.post('/', requireAuth, upload.array('images', MAX_IMAGES), validateImageContent, (req, res) => {
  ...
  logActivity(req, 'product.create', `a créé le produit "${row.name}"`);
...
router.put('/:id', requireAuth, upload.array('images', MAX_IMAGES), validateImageContent, (req, res) => {
  ...
  logActivity(req, 'product.update', `a modifié le produit "${row.name}"`);
  res.json(parseProduct(row));
});
...
router.delete('/:id', requireAuth, (req, res) => {
  ...
  logActivity(req, 'product.delete', `a supprimé le produit "${existing?.name || req.params.id}"`);
  res.json({ ok: true });
});
