...
router.patch('/:id', requireAuth, (req, res) => {
  ...
  if (status && status !== existing.status) {
    logActivity(req, 'order.status', `a changé le statut de la commande ${existing.order_ref} : ${existing.status} → ${newStatus}`);
  } else if (notes !== undefined) {
    logActivity(req, 'order.notes', `a ajouté une note interne sur la commande ${existing.order_ref}`);
  }
  res.json(parseOrder(row));
});
...
router.delete('/:id', requireAuth, (req, res) => {
  const existing = db.prepare('SELECT order_ref FROM orders WHERE id = ?').get(req.params.id);
  ...
  logActivity(req, 'order.delete', `a supprimé la commande ${existing?.order_ref || req.params.id}`);
  res.json({ ok: true });
});
