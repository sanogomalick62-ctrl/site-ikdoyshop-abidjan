...
router.post('/', requireAuth, requireOwner, (req, res) => {
  ...
  logActivity(req, 'user.create', `a créé le compte administrateur "${username}" (${role === 'owner' ? 'propriétaire' : 'admin'})`);
...
router.delete('/:id', requireAuth, requireOwner, (req, res) => {
  ...
  logActivity(req, 'user.delete', `a supprimé le compte administrateur "${existing?.username || req.params.id}"`);
  res.json({ ok: true });
});
